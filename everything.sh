...
GRANT ALL PRIVILEGES ON keystone.* to '$DB_USER'@'$RDO_IP';
EOF
...
sed -e 's/keystone_admin:FreeIPA4All/keystone/' -i  /etc/keystone/keystone.conf

#TODO 163 is EUID for keystone user in IPA, maybe it should be parameterized
mkdir  /var/kerberos/krb5/user/163
chown keystone:keystone  /var/kerberos/krb5/user/163
chmod 700  /var/kerberos/krb5/user/163
ipa-getkeytab -p $IPA_UER@$REALM -k /var/kerberos/krb5/user/163/client.keytab -s $IPA
